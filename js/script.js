$(document).ready(function(){
	var random = 200 + Math.floor(Math.random()* ((700) + 1)); /*500 base*/
	var expanded = 0;
	var canProceed = true;
	var bangDay = "";
	...
	function myDate(m,d,y){
		...
		this.checkDayMonth = function(){
			switch(m){
				case 1: if(d > 31){return false;}
					 case 2: if ((d > 28 && (y % 4 != 0 || y % 400 == 0)) || (d > 29 && y % 4 === 0)){return false;}
		...
		this.subtractDays = function(x){
			...
			for (var i = 1; i >= 0; i--){
				...
				for ( ;j >= 0; j--){
					//alert("J: " + j);
					//alert(this.calendarMonthDate[i][j]);
		...
		this.modSub = 0;
		this.subNum = 265; //this is the weighted average of all time spent in the womb
		this.month = m;
		this.day = d;
		this.year = y;
